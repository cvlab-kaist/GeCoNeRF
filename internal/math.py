...
def convert_ndc_to_raw(w, h, focal, near, coordinate):

  x_ndc, y_ndc, z_ndc = tuple(jnp.moveaxis(coordinate, -1, 0))

  z_raw = (2 * near) / (z_ndc - 1)
  x_raw =  -1 * (w /(2 * focal)) * (x_ndc * z_raw)
  y_raw =  -1 * (h /(2 * focal)) * (y_ndc * z_raw)

  raw_coordinate = jnp.stack([x_raw, y_raw, z_raw], -1)
